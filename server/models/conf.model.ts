...
schema.virtual('raidCount')
    .get(function () {
        return this.parts.filter(p => !['M2expnvme', 'U2expnvme', 'M2Raid', '93008EHBA']
            .includes(p.component?.partNumber)
            && p.component?.type === 'RAID' &&(p.component?.partNumber.match('VROC') || !p.component?.description.match('Модуль'))).reduce((a, b) => a + b.count, 0)
    })
